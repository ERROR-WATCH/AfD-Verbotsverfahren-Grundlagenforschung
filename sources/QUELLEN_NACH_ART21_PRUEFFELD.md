# Quellen nach Art.-21-Prueffeld

## menschenwuerde

Angriffe auf gleiche Wuerde, Rechtssubjektivitaet und staatsbuergerliche Gleichheit.

| Verbotsgrund | Titel | Status |
|---|---|---|
| VG-01 | Verfassungswidrige voelkische Zielsetzung | `BELEGT_UND_VERDRAHTET` |
| VG-02 | Systematische Hetze und Menschenfeindlichkeit | `BELEGT_UND_VERDRAHTET` |
| VG-03 | Antisemitische Codes und Verschwoerungsnarrative | `BELEGT_UND_VERDRAHTET` |
| VG-04 | Verletzung universaler Menschenrechtsprinzipien | `BELEGT_UND_VERDRAHTET` |
| VG-13 | Minderheitenrechte, Remigration und Entrechtungslogik | `BELEGT_UND_VERDRAHTET` |
| VG-17 | Gerichtlich bestaetigte Beobachtungs- und Einstufungsachsen | `BELEGT_UND_VERDRAHTET` |

## demokratieprinzip

Angriffe auf freie politische Willensbildung, parlamentarische Funktionsfaehigkeit, Opposition, Medien und Wahlen.

| Verbotsgrund | Titel | Status |
|---|---|---|
| VG-01 | Verfassungswidrige voelkische Zielsetzung | `BELEGT_UND_VERDRAHTET` |
| VG-03 | Antisemitische Codes und Verschwoerungsnarrative | `BELEGT_UND_VERDRAHTET` |
| VG-10 | Sabotage parlamentarischer Gewaltenteilung und Verfahren | `BELEGT_UND_VERDRAHTET` |
| VG-11 | Untergrabung demokratischer Wahlen, Parteienrecht und Verfahren | `BELEGT_UND_VERDRAHTET` |
| VG-12 | Delegitimierung und Einschuechterung von Justiz, Verfassungsschutz und Presse | `BELEGT_UND_VERDRAHTET` |
| VG-14 | Diffamierung zivilgesellschaftlicher Akteure | `BELEGT_UND_VERDRAHTET` |
| VG-16 | Unwille oder Unfaehigkeit zur innerparteilichen Abgrenzung | `BELEGT_UND_VERDRAHTET` |
| VG-17 | Gerichtlich bestaetigte Beobachtungs- und Einstufungsachsen | `BELEGT_UND_VERDRAHTET` |

## rechtsstaatsprinzip

Angriffe auf Gerichte, Verfahren, Gewaltenteilung, Verwaltung und bindende Rechtsregeln.
Ergänzender Quellenanker 2026-07-23: `dokumente/hauptquellen/Verfassungstreue_Potentialitaet_Quellenanker_2026_07_23.md` dokumentiert Sichert/Friesland als Verfassungstreue- und Wahleignungskontext; nicht als Gerichtsfall gezählt.


| Verbotsgrund | Titel | Status |
|---|---|---|
| VG-04 | Verletzung universaler Menschenrechtsprinzipien | `BELEGT_UND_VERDRAHTET` |
| VG-05 | Personelle Beteiligung an Terror-, Umsturz- und Reichsbuergerkomplexen | `BELEGT_UND_VERDRAHTET` |
| VG-08 | Waffenrecht, Bewaffnung und Sicherheitsrisiko | `BELEGT_UND_VERDRAHTET` |
| VG-09 | Spionage-, Einflussnahme- und Fremdfinanzierungsrisiken im AfD-Umfeld | `BELEGT_UND_VERDRAHTET` |
| VG-10 | Sabotage parlamentarischer Gewaltenteilung und Verfahren | `BELEGT_UND_VERDRAHTET` |
| VG-11 | Untergrabung demokratischer Wahlen, Parteienrecht und Verfahren | `BELEGT_UND_VERDRAHTET` |
| VG-12 | Delegitimierung und Einschuechterung von Justiz, Verfassungsschutz und Presse | `BELEGT_UND_VERDRAHTET` |
| VG-15 | Zugaenge von Verfassungsfeinden zu Parlament, Behoerden und Sicherheitsbereichen | `BELEGT_UND_VERDRAHTET` |
| VG-17 | Gerichtlich bestaetigte Beobachtungs- und Einstufungsachsen | `BELEGT_UND_VERDRAHTET` |

## darauf_ausgehen

Planmaessigkeit, Wiederholung, Organisationsbildung, aggressiv-kaempferische Verdichtung.

| Verbotsgrund | Titel | Status |
|---|---|---|
| VG-02 | Systematische Hetze und Menschenfeindlichkeit | `BELEGT_UND_VERDRAHTET` |
| VG-05 | Personelle Beteiligung an Terror-, Umsturz- und Reichsbuergerkomplexen | `BELEGT_UND_VERDRAHTET` |
| VG-06 | Beziehungen zu rechtsextremen Vorfeld- und Netzwerkstrukturen | `BELEGT_UND_VERDRAHTET` |
| VG-07 | Gewaltbereite und kampfsportorientierte Vorfeldmilieus | `BELEGT_UND_VERDRAHTET` |
| VG-10 | Sabotage parlamentarischer Gewaltenteilung und Verfahren | `BELEGT_UND_VERDRAHTET` |
| VG-12 | Delegitimierung und Einschuechterung von Justiz, Verfassungsschutz und Presse | `BELEGT_UND_VERDRAHTET` |
| VG-13 | Minderheitenrechte, Remigration und Entrechtungslogik | `BELEGT_UND_VERDRAHTET` |
| VG-14 | Diffamierung zivilgesellschaftlicher Akteure | `BELEGT_UND_VERDRAHTET` |
| VG-16 | Unwille oder Unfaehigkeit zur innerparteilichen Abgrenzung | `BELEGT_UND_VERDRAHTET` |
| VG-18 | Durchsetzungskraft: keine marginale Gruppierung | `BELEGT_UND_VERDRAHTET` |

## potentialitaet

Ergänzender Quellenanker 2026-07-23: `dokumente/hauptquellen/Verfassungstreue_Potentialitaet_Quellenanker_2026_07_23.md`, `dokumente/hauptquellen/VBRG_TAZ_Jahresbilanz_Rechte_Gewalt_2025_AfD_Einfluss_Lagebild_2026.md` und `dokumente/hauptquellen/DEMOKRATIE_GEFAEHRDUNG_DEUTSCHLAND_RISIKODOSSIER_2026_07_23.md` verdrahten Brandmauer-/Mehrheitslagen, Parteienbeobachtungsankündigung, Demokratiegefährdung und rechte Gewalt 2025 als Potentialitätskontext ohne automatische AfD-Tatzurechnung.

Ergänzender Quellenanker 2026-07-23: `dokumente/hauptquellen/cdu_brandmauer_deepresearch_2026_07_23/README.md`, `dokumente/hauptquellen/CDU_AfD_Brandmauer_Rechtsverschiebung_Gesamtdossier_2026_07_23.md`, `dokumente/hauptquellen/DEMOKRATIE_GEFAEHRDUNG_DEUTSCHLAND_RISIKODOSSIER_2026_07_23.md` und `dokumente/hauptquellen/CDU_Zentrum_BVP_1933_Steigbuegelhalter_Parallelen_Dossier_2026_07_23.md` dokumentieren CDU/CSU-Brandmauer-, Kretschmer-, Migrations-, Wahlrecht-/Sachsen-Anhalt-Kippszenario-, indirekte-Steigbügelhalter-Mechanik-, WerteUnion/Maaßen-, CDU-/JU-Krefeld-, taz-/Die-Linke- und Zentrum/BVP-1933-Kontext als Potentialitäts- und Normalisierungsanalyse; nicht als AfD-Gerichtsfälle gezählt und nicht als Gleichsetzung.

Reale Durchsetzungskraft: Mitglieder, Mandate, Ressourcen, Reichweite, Vorfeld, Fraktionen, staatliche Mittel.

| Verbotsgrund | Titel | Status |
|---|---|---|
| VG-05 | Personelle Beteiligung an Terror-, Umsturz- und Reichsbuergerkomplexen | `BELEGT_UND_VERDRAHTET` |
| VG-06 | Beziehungen zu rechtsextremen Vorfeld- und Netzwerkstrukturen | `BELEGT_UND_VERDRAHTET` |
| VG-07 | Gewaltbereite und kampfsportorientierte Vorfeldmilieus | `BELEGT_UND_VERDRAHTET` |
| VG-08 | Waffenrecht, Bewaffnung und Sicherheitsrisiko | `BELEGT_UND_VERDRAHTET` |
| VG-09 | Spionage-, Einflussnahme- und Fremdfinanzierungsrisiken im AfD-Umfeld | `BELEGT_UND_VERDRAHTET` |
| VG-11 | Untergrabung demokratischer Wahlen, Parteienrecht und Verfahren | `BELEGT_UND_VERDRAHTET` |
| VG-15 | Zugaenge von Verfassungsfeinden zu Parlament, Behoerden und Sicherheitsbereichen | `BELEGT_UND_VERDRAHTET` |
| VG-18 | Durchsetzungskraft: keine marginale Gruppierung | `BELEGT_UND_VERDRAHTET` |

## zurechnung

Zuordnung eines Befunds zur Partei, Gliederung, Teilorganisation, Funktionaeren, Mitgliedern, Mitarbeitern oder Vorfeld.

| Verbotsgrund | Titel | Status |
|---|---|---|
| VG-01 | Verfassungswidrige voelkische Zielsetzung | `BELEGT_UND_VERDRAHTET` |
| VG-02 | Systematische Hetze und Menschenfeindlichkeit | `BELEGT_UND_VERDRAHTET` |
| VG-03 | Antisemitische Codes und Verschwoerungsnarrative | `BELEGT_UND_VERDRAHTET` |
| VG-04 | Verletzung universaler Menschenrechtsprinzipien | `BELEGT_UND_VERDRAHTET` |
| VG-05 | Personelle Beteiligung an Terror-, Umsturz- und Reichsbuergerkomplexen | `BELEGT_UND_VERDRAHTET` |
| VG-06 | Beziehungen zu rechtsextremen Vorfeld- und Netzwerkstrukturen | `BELEGT_UND_VERDRAHTET` |
| VG-07 | Gewaltbereite und kampfsportorientierte Vorfeldmilieus | `BELEGT_UND_VERDRAHTET` |
| VG-08 | Waffenrecht, Bewaffnung und Sicherheitsrisiko | `BELEGT_UND_VERDRAHTET` |
| VG-09 | Spionage-, Einflussnahme- und Fremdfinanzierungsrisiken im AfD-Umfeld | `BELEGT_UND_VERDRAHTET` |
| VG-13 | Minderheitenrechte, Remigration und Entrechtungslogik | `BELEGT_UND_VERDRAHTET` |
| VG-14 | Diffamierung zivilgesellschaftlicher Akteure | `BELEGT_UND_VERDRAHTET` |
| VG-15 | Zugaenge von Verfassungsfeinden zu Parlament, Behoerden und Sicherheitsbereichen | `BELEGT_UND_VERDRAHTET` |
| VG-16 | Unwille oder Unfaehigkeit zur innerparteilichen Abgrenzung | `BELEGT_UND_VERDRAHTET` |
| VG-17 | Gerichtlich bestaetigte Beobachtungs- und Einstufungsachsen | `BELEGT_UND_VERDRAHTET` |
| VG-18 | Durchsetzungskraft: keine marginale Gruppierung | `BELEGT_UND_VERDRAHTET` |
